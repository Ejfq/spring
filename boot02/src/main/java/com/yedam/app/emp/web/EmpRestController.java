...
public class EmpRestController {
	...
	//단건조회 : GET => emps/100
	@GetMapping("emps/{id}")
	public EmpVO empInfo(@PathVariable(name ="id")Integer employeeId) { // = @PathVariable Integer id
	...
	//등록 	: POST => emps
	@PostMapping("emps")
	public int empInsert(@RequestBody EmpVO empVO) {//@RequestBody => JSON 포맷(Post,Put)
		return empService.empInsert(empVO);
	}
	
	//수정	: PUT => emps/100   	
	@PutMapping("emps/{employeeId}")					//경로를 통해서 수정할 Target			수정할 정보는 JSON포맷
